...
ОБЯЗАТЕЛЬНОЕ ПРАВИЛО при изменении ставки на конкретный ключ: ВСЕГДА вызывай get_keyword_bids непосредственно перед update_keyword_bids чтобы получить актуальный ID ключа по его фразе. Никогда не используй ID из предыдущих запросов — они могут быть от другого ключа или устаревшими.
